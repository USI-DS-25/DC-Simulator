...
class PaxosNode(Node):
    ...
    class AcceptedMsg:
        def __init__(self, acceptor_id, ballot, value):
            self.type = "ACCEPTED"; self.id = acceptor_id; self.ballot = ballot; self.value = value
    class HeartbeatMsg:
    ...
    def __init__(self, node_id, sim, net, all_nodes=None, **kwargs):
        ...
        # State
        self.store.setdefault('promised_ballot', 0)
        self.store.setdefault('accepted_prop', None)
    ...
    def on_message(self, src: int, msg: Any):
        ...
        # 4. ACCEPT (Acceptor Logic)
            ...
            if msg.ballot >= self.store['promised_ballot']:
                ...
                reply = self.AcceptedMsg(self.id, msg.ballot, msg.value)
                for n in self.all_nodes:
                    # DUZELTME
                    self.send(n, reply)

        # 5. ACCEPTED (Commit Logic)
        elif mtype == "ACCEPTED":
            key = (msg.ballot, msg.value)
